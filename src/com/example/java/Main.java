...
public class Main {
    ...
    public static void main(String[] args) {
        try(BufferedReader br = new BufferedReader(new FileReader("src/com/example/java/TRD.csv"))) {
            ...
            LocalTime currentMoment = beginningOfSecond;
            do {
                ...
                sharesInCurrentSecond =+ sharesInTimeMoment.get(currentMoment);
            } while (currentMoment.compareTo(sharesInTimeMoment.lastKey())!=0);

            System.out.println(
                    "maximal amount of shares in 1 second was made between " + secondOfMaxShares[0] + " and " + secondOfMaxShares[1] +
                            ". In this interval " + maxShares + " shares were done.");

            Set<String> exchanges = sharesPerExchange.keySet();
            exchanges.forEach(exchange ->
                   System.out.println("It was done "+sharesPerExchange.get(exchange)+" shares since "+timeMomentOfFirstExchangeShare.get(exchange)+
